...
**Prompts** are the standout feature. They teach your AI how to _think_ about your bookmarks — not just fetch them. Before searching, the AI rewrites your query into better terms. Before saving, it checks for duplicates and suggests tags that fit your existing taxonomy. It can summarise entire reading lists, match fuzzy collection names, and produce weekly digests — all from plain-language requests.

**Caching** keeps things fast and API-friendly. Collections are cached locally with automatic refresh, so repeated requests don't hammer the Raindrop API or burn through your rate limits.
...
| `create_bookmark`  | `link` _(required)_, `title`, `excerpt`, `tags`, `collectionId` |
| `update_bookmark`  | `id` _(required)_, `title`, `excerpt`, `tags`, `collectionId`   |
| `delete_bookmark`  | `id` _(required)_                                               |
...
| ------------------- | -------------------------------------- |
| `list_collections`  | —                                      |
| `create_collection` | `title` _(required)_, `view`, `public` |
| `delete_collection` | `id` _(required)_                      |
